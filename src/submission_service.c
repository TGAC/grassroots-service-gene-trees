...
static const char *GetGeneTreesSubmissionServiceName (const Service *service_p);

static const char *GetGeneTreesSubmissionServiceDescription (const Service *service_p);

static const char *GetGeneTreesSubmissionServiceAlias (const Service *service_p);

static const char *GetGeneTreesSubmissionServiceInformationUri (const Service *service_p);

static ParameterSet *GetGeneTreesSubmissionServiceParameters (Service *service_p, DataResource *resource_p, User *user_p);

static void ReleaseGeneTreesSubmissionServiceParameters (Service *service_p, ParameterSet *params_p);

static ServiceJobSet *RunGeneTreesSubmissionService (Service *service_p, ParameterSet *param_set_p, User *user_p, ProvidersStateTable *providers_p);
...
static ServiceMetadata *GetGeneTreesSubmissionServiceMetadata (Service *service_p);

static bool GetGeneTreesSubmissionServiceParameterTypesForNamedParameters (const Service *service_p, const char *param_name_s, ParameterType *pt_p);
...
static bool AddGenotypesRow (json_t *doc_p, json_t *genotypes_p, GeneTreesServiceData *data_p);

static bson_oid_t *SaveMarkers (const char **parent_a_ss, const char **parent_b_ss, const json_t *data_json_p, GeneTreesServiceData *data_p);

static bool SaveVarieties (const char *parent_a_s, const char *parent_b_s, const bson_oid_t *id_p, GeneTreesServiceData *data_p);

static bool SaveVariety (const char *parent_s, const bson_oid_t *id_p, MongoTool *mongo_p);

static char *GetAccession (const json_t *genotypes_p, GeneTreesServiceData *data_p);
...
Service *GetGeneTreesSubmissionService (GrassrootsServer *grassroots_p)
{
			...
			if (data_p)
				{
					if (InitialiseService (service_p,
																 GetGeneTreesSubmissionServiceName,
																 GetGeneTreesSubmissionServiceDescription,
																 GetGeneTreesSubmissionServiceAlias,
																 GetGeneTreesSubmissionServiceInformationUri,
																 RunGeneTreesSubmissionService,
																 IsResourceForGeneTreesSubmissionService,
																 GetGeneTreesSubmissionServiceParameters,
																 GetGeneTreesSubmissionServiceParameterTypesForNamedParameters,
																 ReleaseGeneTreesSubmissionServiceParameters,
																 CloseGeneTreesSubmissionService,
																 NULL,
																 false,
																 SY_SYNCHRONOUS,
																 (ServiceData *) data_p,
																 GetGeneTreesSubmissionServiceMetadata,
																 NULL,
																 grassroots_p))
						{
...
static const char *GetGeneTreesSubmissionServiceName (const Service * UNUSED_PARAM (service_p))
{
	return "GeneTrees submission service";
}


static const char *GetGeneTreesSubmissionServiceDescription (const Service * UNUSED_PARAM (service_p))
{
	return "A service to submit parental-cross genotype data";
}


static const char *GetGeneTreesSubmissionServiceAlias (const Service * UNUSED_PARAM (service_p))
{
	return GT_GROUP_ALIAS_PREFIX_S SERVICE_GROUP_ALIAS_SEPARATOR "submit";
}


static const char *GetGeneTreesSubmissionServiceInformationUri (const Service * UNUSED_PARAM (service_p))
{
	return NULL;
}


static ParameterSet *GetGeneTreesSubmissionServiceParameters (Service *service_p, DataResource * UNUSED_PARAM (resource_p), User * UNUSED_PARAM (user_p))
{
	...
	if (param_set_p)
		{
			...
			if ((param_p = EasyCreateAndAddJSONParameterToParameterSet (data_p, param_set_p, group_p, S_SET_DATA.npt_type, S_SET_DATA.npt_name_s, "Data", "The parental-cross data", NULL, PL_ALL)) != NULL)
				{
		...
		{
			PrintErrors (STM_LEVEL_SEVERE, __FILE__, __LINE__, "Failed to allocate %s ParameterSet", GetGeneTreesSubmissionServiceName (service_p));
		}
...
static void ReleaseGeneTreesSubmissionServiceParameters (Service * UNUSED_PARAM (service_p), ParameterSet *params_p)
{
...
static ServiceJobSet *RunGeneTreesSubmissionService (Service *service_p, ParameterSet *param_set_p, User * UNUSED_PARAM (user_p), ProvidersStateTable * UNUSED_PARAM (providers_p))
{
